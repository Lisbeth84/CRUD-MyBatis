    ...
    $.ajax({
        ...
        success: function (x) {
            ...
            x.forEach((item, index, array) => {
                $("#tablita").append(
                        "<tr><td>" + (index + 1) + "</td><td>" + item.id + "</td><td>" + item.nombre
...
$("#imagen").change(function (e) {
    imagen = e.target.productos[0].name;
...
$("#guardar").click(function () {
    ...
    //alert(foto)
     ...
     data: JSON.stringify({'nombres': nombre, 'precio': precio, 'cantidad':cantidad, 'imagen':imagen}),
     cache: false,
     success: function (w) {
     bootbox.alert({
